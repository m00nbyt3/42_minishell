...
/*   Updated: 2022/05/17 16:33:17 by ycarro           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_cd(t_transformer *runner, t_env *env)
{
	...
	{
		path = 0;
		path = get_my_env("HOME", env->array);
		if (!path)
		{
	...
	if (ft_strcmp(path, "-"))
		path = go_to_last_path(path, env->array);
	if (chdir(path) == -1)
	{
		file_not_found(path);
		return ;
...
void	mod_env_pwd(t_env *env)
{
	...
	path = get_my_env("PWD", env->array);
	toadd1 = ft_strjoin("OLDPWD=", path);
	free(path);
	path = getcwd(0, 0);
	toadd2 = ft_strjoin("PWD=", path);
	free(path);
	if (!var_exist("OLDPWD=", env->array, -1, 0))
		replace_env(toadd1, env->array);
	else
		env->array = ft_env_add(toadd1, env->array);
	if (!var_exist("PWD=", env->array, -1, 0))
		replace_env(toadd2, env->array);
	else
		env->array = ft_env_add(toadd2, env->array);
	free(toadd1);
	free(toadd2);
}
